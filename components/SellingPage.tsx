...
export default function SellingPage({ onContinue, onBack }: SellingPageProps) {
  ...
  useEffect(() => {
    ...
    if (curvePath) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          // Create vertical dashed line element
          // Coordinates: Week 4 point (x=500, y=270) to green dot (x=619.466, y=45.925)
          const verticalLine = document.createElementNS('http://www.w3.org/2000/svg', 'line')
          verticalLine.setAttribute('x1', '500')
          verticalLine.setAttribute('y1', '270')
          verticalLine.setAttribute('x2', '619.466')
          verticalLine.setAttribute('y2', '45.925')
          ...
          setTimeout(() => {
            ...
            const lineLength = Math.sqrt(Math.pow(619.466 - 500, 2) + Math.pow(45.925 - 270, 2))
  ...
  return (
    <div className="container max-w-[480px] mx-auto p-0 min-h-screen flex flex-col pb-[100px] bg-white">
      {/* Header */}
      <div className="header px-6 py-5 flex items-center justify-between bg-white relative">
        {/* Back Button */}
        {onBack && (
          <button
            type="button"
            onClick={onBack}
            className="absolute left-4 top-[50%] -translate-y-1/2 cursor-pointer border-0 bg-transparent p-0 z-10"
            aria-label="Go back"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" className="fill-[#212121]">
        ...
        {/* Logo - SVG from other quiz sections */}
        ...
        {/* Hamburger Menu */}
        <div className="menu-icon w-8 h-8 flex flex-col justify-center gap-1.5 cursor-pointer">
          <span className="w-full h-[3px] bg-[#212121] rounded-sm"></span>
          <span className="w-full h-[3px] bg-[#212121] rounded-sm"></span>
          <span className="w-full h-[3px] bg-[#212121] rounded-sm"></span>
        </div>
      </div>
      ...
      <div className="readiness-section px-6 pt-8 pb-6">
        <h2 className="section-label mb-8 text-left text-lg font-medium text-[#212121]" style={{ fontFamily: "'Space Grotesk', Arial, sans-serif" }}>
          Your readiness level
        </h2>
        
        {/* Chart Container */}
        <div ref={chartContainerRef} className="chart-container relative mx-auto w-full max-w-lg py-10" style={{ minHeight: '300px' }}>
          {svgContent && (
            <div
        ...
        {/* Disclaimer */}
        <p className="disclaimer mt-4 text-xs italic text-[#6B7280] text-center" style={{ fontFamily: "'Space Grotesk', Arial, sans-serif" }}>
          This chart is for illustrative purposes only
        </p>
      </div>

      {/* Main Title */}
      <div className="main-title-section px-6 py-6 text-center">
        <h1 className="main-title text-[32px] font-bold leading-tight text-[#1E1B4B] mb-0" style={{ fontFamily: "'Space Grotesk', Arial, sans-serif" }}>
          Your 4-week Personal AI Challenge is ready!
        </h1>
      </div>

      {/* Fixed Continue Button */}
      <div className="continue-btn-wrapper fixed bottom-0 left-0 right-0 px-6 py-4 bg-white shadow-[0_-4px_12px_rgba(0,0,0,0.08)] z-[100]">
        <button
          onClick={onContinue}
          className="continue-btn w-full max-w-[480px] mx-auto block py-[18px] bg-[#5653FE] text-white border-none rounded-xl text-base font-bold cursor-pointer transition-all duration-300 uppercase tracking-wider hover:bg-[#4442D9] hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(86,83,254,0.4)] active:translate-y-0"
